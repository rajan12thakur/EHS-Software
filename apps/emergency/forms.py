...
class EmergencySessionTrainerForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "trainer_name": forms.TextInput(attrs={**TEXT_INPUT, "placeholder": "Trainer name"}),
            "trainer_designation": forms.TextInput(
                attrs={**TEXT_INPUT, "placeholder": "e.g. Safety Officer"}
            ),
            "trainer_department": forms.Select(attrs=SELECT),
            "trainer_is_external": forms.CheckboxInput(attrs=CHECKBOX),
            "trainer_organization": forms.TextInput(
                attrs={**TEXT_INPUT, "placeholder": "Required for external trainers"}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["trainer_organization"].required = False
    ...
    def clean(self):
        ...
        trainer_organization = (cleaned_data.get("trainer_organization") or "").strip()

        if not trainer_name:
            return cleaned_data

        if trainer_is_external and not trainer_organization:
            self.add_error("trainer_organization", "Organization is required for external trainers.")

        if not trainer_is_external and not trainer_department:
            self.add_error("trainer_department", "Department is required for internal trainers.")
    ...
    def clean(self):
        ...
        for form in self.forms:
            ...
            if form.cleaned_data and not form.cleaned_data.get("DELETE", False):
                if form.cleaned_data.get("trainer_name"):
                    active_forms += 1
        if active_forms == 0:
